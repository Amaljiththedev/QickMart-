...
def reset_password(request):
    if request.method == "POST":
        ...
        if not re.match(pattern_password, password):
            messages.error(request, "Password is too weak. It must contain at least one lowercase letter, one uppercase letter, one digit, and be at least 8 characters long.")
            return redirect('reset_password')  # Redirect back to reset password page with error message
